      ...
      fun ` (base, A { align=a, offset, store, loadA, loadV }) = A {
         ...
         store = fn finish => fn x => store (fn writer => finish (fn (a, offset, i) =>
            let
               ...
               (* padding: should become a noop if offset = i *)
               (*
               val subarr = 
                  Word8ArraySlice.slice (a, offset, SOME (offset' - offset))
               val () = Word8ArraySlice.modify (fn _ => 0w0) subarr
               *)
      ...
      fun pickle x =
         let
            fun F (A { align, offset, store, loadV, loadA }) = {
               ...
               parseVector = fn (v, f) => 
                  if Word8Vector.length v < offset 0 then raise Subscript else
                  loadV (v, 0, 0, f),
