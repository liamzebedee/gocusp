...
(*
      val debug = fn
         READER (DATA { eof, offset, length }, { id }) =>
            print (" data          id=" ^ Word16.toString id ^
                   " eof=" ^ Bool.toString eof ^ 
                   " offset=" ^ (if offset then "?" else "0") ^ 
                   " length=" ^ Int.toString length ^ "\n")
       | READER (READER_CTL WRITER_RESETS, { id }) => 
            print (" writer resets id=" ^ Word16.toString id ^ "\n")
       | WRITER (WRITER_CTL READER_RESETS, { id, seq }) =>
            print (" reader reset  id=" ^ Word16.toString id ^ 
                   " seq=" ^ Int.toString seq ^ "\n")
       | WRITER (WRITER_CTL READER_BARRIER, { id, seq }) =>
            print (" reader barrier  id=" ^ Word16.toString id ^ 
                   " seq=" ^ Int.toString seq ^ "\n")
       | CREATE (CONNECT, { service, seq }) =>
            print (" attach   service=" ^ Word16.toString service ^
                   " seq=" ^ Int.toString seq ^ "\n")
       | CORRUPT => 
            print " CORUPT!\n"
      
      val parse = fn d => let val out = parse d in debug out; out end
      val write = fn (d, m) => (debug m; write (d, m))
